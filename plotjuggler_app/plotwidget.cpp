...
{
    connect(this, &PlotWidget::curveListChanged, this, [this]() {
        this->updateMaximumZoomArea();
        qDebug() << "this is running<<" << endl;
    });
...
PlotWidget::CurveInfo* PlotWidget::addCurve(const std::string& name, QColor color )
{
  ...
  try {
      auto plot_qwt = createTimeSeries(
          "", &data); //it create time series here using data(by ref) from dataMap().numeric.find...
                      //base on mapped data
      //put name here and get
      curve->setPaintAttribute(QwtPlotCurve::ClipPolygons, true);
      curve->setPaintAttribute(QwtPlotCurve::FilterPointsAggressive, true);
      curve->setData(plot_qwt); //input data
  ...
  //something to handle this marker and store it at qwt
...
{
    qDebug() << "DRAG BITCH" << endl;
    const QMimeData *mimeData = event->mimeData();
    QStringList mimeFormats = mimeData->formats();
    _dragging.curves.clear();
    _dragging.source = event->source(); //get source

    for (const QString &format : mimeFormats) {
        QByteArray encoded = mimeData->data(format);
        QDataStream stream(&encoded, QIODevice::ReadOnly);

        while (!stream.atEnd()) { //fuck curve_name first
            QString curve_name;
            stream >> curve_name;
            qDebug() << "DRAG BITCH data" << curve_name << endl;
            qDebug() << "DRAG format" << format << endl;
        ...
        if (format == "curveslist/add_string") {
            _dragging.mode = DragInfo::STRINGS;
            qDebug() << "add_string yeahhhh" << endl;
            event->acceptProposedAction();
        }
    }
}
...
void PlotWidget::dropEvent(QDropEvent *)
{
    ...
    if (_dragging.mode == DragInfo::CURVES) {
        if (isXYPlot() && !_curve_list.empty()) {
            _dragging.mode = DragInfo::NONE;
            _dragging.curves.clear(); //dragging.curves should store name of curve i think i dontknow
...
void PlotWidget::addLabel(const std::string &label_name)
{
    ...
    //plot all labels in strings
    auto iter = _mapped_data.strings.find(label_name); //ok
    ...
    qDebug() << "brah" << QString::fromStdString(label_name) << endl;
...
void PlotWidget::updateMaximumZoomArea()
{
    ...
    auto rangeXnum = getMaximumRangeXNumeric();
    auto rangeY = getMaximumRangeY(rangeXnum);
    max_rect.setBottom(rangeY.min);
    max_rect.setTop(rangeY.max);
    //    bool isLabelPlot = label.size() != 0;
    ...
    qDebug() << "max_rect.left()" << max_rect.left() << "max_rect.right()" << max_rect.right();
...
void PlotWidget::setZoomRectangle(QRectF rect, bool emit_signal)
{
    ...
    qDebug() << "I set zoom rect for real" << endl;
    ...
    this->setAxisScale(xBottom,
                       ...
                       std::max(rect.left(), rect.right()));
    this->updateAxes();
    qDebug() << "I update Axis for real" << endl;

    if (isXYPlot() && _keep_aspect_ratio) {
        rescaleEqualAxisScaling(); //debug
    }
...
Range PlotWidget::getMaximumRangeXNumeric() const
{
...
Range PlotWidget::getMaximumRangeX() const
{
    ...
    Range r = getMaximumRangeXNumeric();
    left = r.min;
    right = r.max;
...
Range PlotWidget::getMaximumRangeY(Range range_X) const
{
    ...
    for (auto &it : _curve_list) {
        ...
        //       double left = range_X.min;
        //        double right = range_X.max;
    ...
    //  qDebug() << "bottom" << bottom << "top" << top << endl;
...
void PlotWidget::updateCurves()
{
    for (auto &it : _curve_list) {
        ...
        //qDebug() << "this is x" << it.curve->data()->sample(1).x() << endl;
        // qDebug() << "this is y" << it.curve->data()->sample(1).y() << endl;
...
{
    double add_time = 0;
    PJ::StringSeries *str_series;
    for (auto &lb : label) {
        str_series = &_mapped_data.strings.find(lb.second->name)->second;
        for (auto &str : *str_series) {
            if (!lb.second->markersContain(str.x)) { // if not find in markers
                add_time = str.x;
                addLabelMarker(*lb.second, str.y.data(), add_time);
                qDebug() << "addLabelMarker" << endl;
                updateMaximumZoomArea();
                qDebug() << "addLabelMarker" << endl;
            };
        }
...
{
    if (_curve_list.size() == 0 && label.size() == 0) {
        QRectF rect(0, 1, 1, -1);
        this->setZoomRectangle(rect, false);
        return;
    }
