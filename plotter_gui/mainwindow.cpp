...
void MainWindow::onUndoableChange() {
	...
	if (elapsed_ms < 100) {
		if (_undo_states.empty() == false) _undo_states.pop_back();
	}
...
void MainWindow::resizeEvent(QResizeEvent*) { on_splitterMoved(0, 0); }
...
void MainWindow::checkAllCurvesFromLayout(const QDomElement& root) {
	...
	if (missing_curves.size() > 0) {
		...
		msgBox.setText(
			tr("One or more timeseries in the layout haven't been loaded yet\n"
...
void importPlotDataMapHelper(std::unordered_map<std::string, T>& source,
	...
	for (auto& it : source) {
		...
		if (plot_with_same_name == destination.end()) {
			plot_with_same_name =
				destination.emplace(std::piecewise_construct, std::forward_as_tuple(name), std::forward_as_tuple(name))
					.first;
		}
...
bool MainWindow::isStreamingActive() const { return ui->pushButtonStreaming->isChecked() && _current_streamer; }
...
bool MainWindow::loadDataFromFiles(QStringList filenames) {
	if (filenames.size() > 1) {
		...
		msgbox.setText(
			"You are loading multiple files at once. A prefix will be automatically added to the name of the "
			"timeseries.\n\n"
...
QString MainWindow::styleDirectory() const { return _style_directory; }
...
void MainWindow::loadPluginState(const QDomElement& root) {
	...
	for (QDomElement plugin_elem = plugins.firstChildElement(); plugin_elem.isNull() == false;
		...
		if (plugin_elem.nodeName() != "plugin" || plugin_name.isEmpty()) {
			QMessageBox::warning(
				this, tr("Error loading Plugin State from Layout"),
...
QDomElement MainWindow::savePluginState(QDomDocument& doc) {
	...
	auto CheckValidFormat = [this](const QString& expected_name, const QDomElement& elem) {
		if (elem.nodeName() != "plugin" || elem.attribute("ID") != expected_name) {
			QMessageBox::warning(
				this, tr("Error saving Plugin State to Layout"),
...
bool MainWindow::loadLayoutFromFile(QString filename) {
	...
	if (!snippets_element.isNull()) {
		...
		if (snippets_are_different) {
			...
			msgBox.setText(
				"Your layour file contains a set of custom transforms different from "
...
void MainWindow::on_tabbedAreaDestroyed(QObject* object) { this->setFocus(); }
...
void MainWindow::onCreateFloatingWindow(PlotMatrix* first_tab) { createTabbedDialog(QString(), first_tab); }
...
void MainWindow::updateDataAndReplot(bool replot_hidden_tabs) {
	if (_current_streamer) {
		...
		qDebug() << "listSize:" << curvelist_added.size();
...
void MainWindow::on_pushButtonOptions_toggled(bool checked) { ui->widgetOptions->setVisible(checked); }
...
void MainWindow::on_pushButtonUseDateTime_toggled(bool checked) { updatedDisplayTime(); }
...
void MainWindow::on_addMathPlot(const std::string& linked_name) { addOrEditMathPlot(linked_name, false); }

void MainWindow::on_editMathPlot(const std::string& plot_name) { addOrEditMathPlot(plot_name, true); }
...

{
	...
	checkbox_datasource->setToolTip(
...
void MainWindow::on_actionLoadDummyData_triggered() { buildDummyData(); }
